...
@gdaltest.enable_exceptions()
def test_zarr_read_sharding_then_byte_byte_codec():
    ...
    with gdaltest.error_raised(
        ...
        ds = gdal.OpenEx(
            "data/zarr/v3/sharding_then_byte_byte_codec.zarr",
            gdal.OF_MULTIDIM_RASTER,
        )
        ar = ds.GetRootGroup().OpenMDArray("sharding_then_byte_byte_codec")
...
@gdaltest.enable_exceptions()
def test_zarr_read_sharding_then_byte_byte_codec_read_errors(tmp_vsimem):
    ...
    data = bytearray(
        open("data/zarr/v3/sharding_then_byte_byte_codec.zarr/c/0/0", "rb").read()
    )

    # Corrupt last byte (CRC32c)
    data[-1] = 0
    gdal.FileFromMemBuffer(
        tmp_vsimem / "sharding_then_byte_byte_codec.zarr/c/0/0", data
    )
    ...
    with gdaltest.error_raised(
        ...
        ds = gdal.OpenEx(
            tmp_vsimem / "sharding_then_byte_byte_codec.zarr",
            gdal.OF_MULTIDIM_RASTER,
        )
        ar = ds.GetRootGroup().OpenMDArray("sharding_then_byte_byte_codec")
