...
/************************************************************************/
/*            GDALMdimConcatAlgorithm::GDALMdimConcatAlgorithm()        */
/************************************************************************/
    ...
    : GDALAlgorithm(NAME, DESCRIPTION, HELP_URL)
{
    ...
    AddInputDatasetArg(&m_inputDatasets,
    ...
    {
        auto &arg =
            AddArg("array", 0, _("Name of array to concatenate."), &m_array);
...
std::optional<GDALMdimConcatAlgorithm::DimensionDesc>
GDALMdimConcatAlgorithm::GetDimensionDesc(
    ...
    const std::shared_ptr<GDALDimension> &poDim) const
{
    ...
    CPLAssert(nSize > 0);
    desc.nSize = nSize;
    if (!poVar->IsRegularlySpaced(desc.dfStart, desc.dfIncrement))
    {
...
/************************************************************************/
/*            GDALMdimConcatAlgorithm::BuildDimensionDesc()             */
/************************************************************************/

std::optional<std::vector<GDALMdimConcatAlgorithm::DimensionDesc>>
GDALMdimConcatAlgorithm::BuildDimensionDesc(
    ...
    std::vector<std::vector<SourceShortDimDesc>> &aaoSourceShortDimDesc)
{
    ...
    for (const auto &dataset : m_inputDatasets)
    {
        ...
        {
            ReportError(CE_Failure, CPLE_AppDefined,
                        "Array %s of dataset %s has not dimensions",
                        m_array.c_str(), dataset.GetName().c_str());
            return std::nullopt;
        }
...
/************************************************************************/
/*                   GDALMdimConcatAlgorithm::RunImpl()                 */
/************************************************************************/

bool GDALMdimConcatAlgorithm::RunImpl(GDALProgressFunc pfnProgress,
                                      void *pProgressData)
{
    ...
    // Create concatenated array dimensions
    ...
    // Add sources to concatenated array
