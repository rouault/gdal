    ...
    else if (*pszInput >= '0' && *pszInput <= '9')
    {
        ...
        else
        {
            ...
            if (osToken.size() >= 12 || nVal < INT_MIN || nVal > INT_MAX)
                ...
                *ppNode = new GDAL_c_expr_node(static_cast<int>(nVal));
        }
...
{
    for (const auto &otherSubExpr : other.papoSubExpr)
    {
        papoSubExpr.emplace_back(
...
{
    papoSubExpr.push_back(std::unique_ptr<GDAL_c_expr_node>(child));
}
...
{
    std::reverse(papoSubExpr.begin(), papoSubExpr.end());
}
